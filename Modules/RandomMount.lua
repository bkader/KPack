...
KPack:AddModule("RandomMount", function(folder, core)
    if core:IsDisabled("RandomMount") then return end
        ...
        for i = 1, GetNumCompanions("MOUNT"), 1 do
            ...
            if strfind(creatureName, "Qiraji") then
                ...
                if mountDict[creatureSpellID] then
                    if
                        ((mountDict[creatureSpellID][1] / 8) % 2) >= 1 or
                            ((mountDict[creatureSpellID][1] / 16) % 2) >= 1
                     then
                        tinsert(flying, i)
                    end
                else
    ...
    local function TestMount(r, ridingSK, zoneText, canFly, flying, qiraji, searchMount, inLocation, repeatChk, zoneChk)
        ...
        if ridingSK >= 75 then
            ...
            if r ~= -1 and search == "" and runawayChk == 500 then
                ...
                repeat
                    ...
                    s = TestMount(r, ridingSK, zoneText, canFly, flying, qiraji, searchMount, inLocation, repeatChk, zoneChk)
...
-- bindings
BINDING_HEADER_KPACKRANDOMMOUNT = "|cfff58cbaK|r|caaf49141Pack|r Random Mount"
