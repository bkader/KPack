...
local mod=addon.QuickButton
if not mod then
  mod={}
  addon.QuickButton=mod
end
...
local function QuickButton_DestroyButton()
  ...
  if _G[btnName] then _G[btnName]:Hide() end
end
...
local function EventHandler(self, event, ...)
  if event=="ADDON_LOADED" then
    ...
    if name:lower()==addonName:lower() then
      ...
      if not classes[unitClass] or not LibQTip then return end
  ...
  elseif type(f[event])=="function" then
...
do
  ...
  do
    ...
    function QuickButton_CreateButton()
      ...
      button=button or CreateFrame("Button", btnName, UIParent, "SecureActionButtonTemplate, ActionButtonTemplate")
  ...
  if not IsInParty then
    IsInParty =function() return (GetNumPartyMembers() > 0) end
  end

  local IsInGroup =_G.IsInGroup
  if not IsInGroup then
    IsInGroup =function() return (IsInRaid() or IsInParty()) end
  end
  ...
  function f:PLAYER_ENTERING_WORLD()
    ...
    if QuickButtonDB.enabled and button then
      ...
      ShowHide(button, IsUsableSpell(spellName))
  ...
  f.SPELLS_CHANGED=f.PLAYER_ENTERING_WORLD

  -- on PLAYER_TALENT_UPDATE event
  function f:PLAYER_TALENT_UPDATE()
    QuickButton_Initialize()
  end
  f.RAID_ROSTER_UPDATE=f.PLAYER_TALENT_UPDATE
  f.PARTY_MEMBERS_CHANGED=f.PLAYER_TALENT_UPDATE

  function f:UPDATE_BINDINGS()
