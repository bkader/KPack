...
KPack:AddModule("RaidTargetIcons", function(_, core)
	if core:IsDisabled("RaidTargetIcons") then return end
	        ...
	        if not self.hiding then
	            ...
	            if self.hovering then
	                if self.index and (not saved or saved == self.index) then
	                    ...
	                    if self.hovering > (config.hovertime or 0.2) then
	                        self:Click()
	                    end
	                else
	                    self.hovering = 0
	...
	function RTI.AreModifiersDown()
	    if not config.ctrl and not config.alt and not config.shift then
	        return false
	    elseif config.ctrl ~= (IsControlKeyDown() == 1) then
	        return false
	    elseif config.alt ~= (IsAltKeyDown() == 1) then
	        return false
	    elseif config.shift ~= (IsShiftKeyDown() == 1) then
	        return false
	    end
	    return true
	end
	...
	hooksecurefunc("SetRaidTarget", function(unit, index) RTI.SetRaidTarget(index, unit, 1) end)
	...
	function RTI.OnMouseUp(frame, btn)
	    if btn == "LeftButton" then
	        ...
	        local double = (config.double and RTI.click and curtime - RTI.click < (config.speed or 0.25) and abs(x - RTI.clickX) < 20 and abs(y - RTI.clickY) < 20)
	        if modifiers or double then
	            if (modifiers and config.singlehover) or (double and config.doublehover) then
