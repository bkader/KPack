...
function CreateDeathRecapFrame()
    if KPackDeathRecapFrame then return end
    ...
    StaticPopupDialogs["DEATH"] = {
        ...
        OnAccept = function(self)
            ...
            RepopMe()
            if CannotBeResurrected() then return 1 end
        end,
        ...
        DisplayButton2 = function(self) return HasSoulstone() end,
        DisplayButton3 = function(self) return HasEvents() end,
        timeout = 0,
        whileDead = 1,
...
E.RESURRECT_REQUEST = mod.HideDeathPopup
E.PLAYER_ALIVE = mod.HideDeathPopup
E.RAISED_AS_GHOUL = mod.HideDeathPopup

function E:PLAYER_DEAD()
...
function E:COMBAT_LOG_EVENT_UNFILTERED(timestamp, event, _, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
    if (band(dstFlags, COMBATLOG_FILTER_ME) ~= COMBATLOG_FILTER_ME) or (band(srcFlags, COMBATLOG_FILTER_ME) == COMBATLOG_FILTER_ME) then
        return
    end
    if not validEvents[event] then return end
